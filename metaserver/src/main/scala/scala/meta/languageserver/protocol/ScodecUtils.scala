...
object ScodecUtils {
  ...
  def bracketedBy(prefix: String, terminator: String): Codec[String] = filtered(ascii, new Codec[BitVector] {
    ...
    override def encode(bits: BitVector): Attempt[BitVector] = Attempt.successful(startBits ++ bits ++ terminatorBits)
    override def decode(bits: BitVector): Attempt[DecodeResult[BitVector]] = {
      val bitsLen = bits.length
      bits.acquire(startBitsLen) match {
        case Right(aquired) if (aquired == startBits) => bits.drop(startBitsLen).indexOfSlice(terminatorBits) match {
          case -1 =>
            if (bitsLen >= maxLen) Attempt.failure(Err("Does not contain terminator."))
            else Attempt.failure(Err(s"terminator not found in $bitsLen bits"))
          case tIdx =>
            val found = bits.slice(startBitsLen, startBitsLen + tIdx)
            val remainder = bits.drop(startBitsLen + tIdx + terminatorBitsLen)
            Attempt.successful(DecodeResult(found, remainder))
        }
        case Right(aquired) => Attempt.failure(Err("Does not start with prefix."))
