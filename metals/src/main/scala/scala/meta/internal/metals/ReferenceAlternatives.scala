...
case class ReferenceAlternatives(
    ...
    locals: Map[Path, Set[String]]
)

object ReferenceAlternatives {
  private val isCopyOrApply = Set("apply", "copy")
  ...
  def apply(
    ...
    val candidates = for {
      ...
      if info.symbol != name
      if {
        isVarSetter(occ, name, info) ||
        isCompanionObject(occ, name, info) ||
        isCopyOrApplyParam(occ, name, info)
      }
    } yield info.symbol
  ...
  private def isCopyOrApplyParam(
      occ: String,
      name: String,
      info: SymbolInformation
  ): Boolean =
