...
object Symbol {
  ...
  def validated(sym: String): Either[String, Symbol] = {
    ...
    // at least.
    // The underlying symbol parsed throws exceptions, the syntax is so simple
    // it should be easy to write one from scratch but I don't feel like doing it right now.
